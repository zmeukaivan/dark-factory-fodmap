...
| 2026-08-19 | Prime node (issue #7) hung ~12 min with no output — full tool access let it run the full gate and spawn a browser. Killed it; the runner logged `NODE_FAILED` and continued to `plan`, which re-derived the priming context and the lap completed | Added `FACTORY_NODE_TIMEOUT_MINUTES` (a 15 min `timeout` leash on every node) and hardened `prime.md` against running the full gate; the tool-allowlist drop in `opencode-agent.sh` remains, but a hang is now bounded |
