...
The structural gate has already run. It found the app started, the E2E asserted its steps,
the holdout scenarios passed, all deliberate defects were caught, and no protected file
was touched.
...
- Is anything here unrelated to the issue? Scope creep is a block even when the extra code
  is good.
- Does it touch food data without a source, or alter a rating? That is a block
  (`FACTORY_RULES.md` §2.8): a wrong rating causes real symptoms.
...
Block on: wrong behaviour, a weakened assertion, scope creep, an invariant breach, a
wrong food rating. Do not block on: naming, formatting, a comment you would have worded
...
```json
{
  ...
  "issues_to_fix": [
    {"severity": "critical|high|medium|low",
     "category": "correctness|scope|observability|invariant|data",
